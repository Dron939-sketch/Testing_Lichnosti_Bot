...
def update_free_used_in_api(user_id: int) -> bool:
    ...
    try:
        logger.info(f"🔄 Вызов update_free_used_in_api для user_id={user_id}")
        ...
        if response.status_code == 200:
            ...
            logger.warning(f"⚠️ Ошибка обновления free_used: {response.status_code} - {response.text}")
            return False
    except Exception as e:
...
async def send_invite_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ДЛЯ ОТПРАВКИ ДРУГУ - полный текст со ссылкой (нормальный, без кракозябр)
        full_text = f"{invite_text}\n\n🔗 {invite_url}"
        share_url = f"tg://msg_url?url={urllib.parse.quote(' ')}&text={urllib.parse.quote(full_text)}"
