...
ВЕРСИЯ 6.1: ИСПРАВЛЕНА ОШИБКА С NOOP_CALLBACK
...
try:
    ...
    # Диагностика импорта
    logger.info(f"🔍 ДИАГНОСТИКА ИМПОРТА:")
    logger.info(f"  handle_stage_1_answer: {handle_stage_1_answer}")
    logger.info(f"  Тип: {type(handle_stage_1_answer)}")
    logger.info(f"  Это функция: {callable(handle_stage_1_answer)}")
    
except ImportError as e:
    logger.error(f"❌ Ошибка импорта обработчиков теста: {e}")
    # Создаем реальные заглушки, которые хотя бы логируют вызов
...
async def show_results_screen(
    ...
    # ИСПРАВЛЕНО: изменен callback_data с "my_sexual_profile" на "show_my_sexual_profile"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and context.args[0].startswith("sex_"):
        ...
        # Временно заглушка для sexual_invite_start
        await update.message.reply_text(
            f"🔞 Приглашение в 18+ модуль получено!\n\n"
            f"Код приглашения: {context.args[0]}\n\n"
...
async def show_my_sexual_profile_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"❌ Ошибка в show_my_sexual_profile_callback: {e}\n{traceback.format_exc()}")
        try:
...
def main():
    ...
    print("\n" + "="*70)
    print("🧠 ВИРТУАЛЬНЫЙ ПСИХОЛОГ ВАРИАТИКА (ВЕРСИЯ 6.1)")
    print("="*70)
    ...
    print("="*70)
    print("🔧 ИСПРАВЛЕНИЯ В 6.1:")
    print("   ✅ Добавлена функция noop_callback")
    print("   ✅ Исправлен callback_data для 18+ кнопки")
    print("   ✅ Добавлена диагностика импортов")
    print("="*70)
    ...
    print(f"Тип: {type(handle_stage_1_answer)}")
    print(f"Это функция: {callable(handle_stage_1_answer)}")
    print(f"Имя: {handle_stage_1_answer.__name__}")
    print("="*50)
    ...
    # Заглушки для 18+ модуля (исправлено - добавлена функция noop_callback)
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            RESULTS: [
                ...
                # ИСПРАВЛЕНО: изменен pattern с "show_my_sexual_profile" на "my_sexual_profile"
                CallbackQueryHandler(show_my_sexual_profile_callback, pattern="^my_sexual_profile$"),
            ],
            GIFT_SCREEN: [
                CallbackQueryHandler(confirm_share, pattern="^confirm_share$"),
                CallbackQueryHandler(skip_share, pattern="^skip_share$"),
            ...
            SEXUAL_PROFILE_SCREEN: [
                CallbackQueryHandler(show_my_sexual_profile_callback, pattern="^show_my_sexual_profile$"),
                CallbackQueryHandler(create_invite_callback, pattern="^create_invite$"),
                CallbackQueryHandler(my_invites_callback, pattern="^my_invites$"),
                CallbackQueryHandler(back_to_results, pattern="^back_to_results$"),
            ],
            ...
            FOUR_F_PAYMENT_SCREEN: [
                ...
                CallbackQueryHandler(noop_callback, pattern="^check_payment_"),
                CallbackQueryHandler(noop_callback, pattern="^pay_access_"),
    ...
    logger.info("✅ ВЕРСИЯ 6.1: ИСПРАВЛЕНА ОШИБКА С NOOP_CALLBACK!")
