...
4 этапа + адаптивные уточнения + Telegram спойлеры
"""
...
# ============================================
# ✅ ОБНОВЛЕННАЯ ФУНКЦИЯ: show_results_screen СО СПОЙЛЕРАМИ
# ============================================

async def show_results_screen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """✅ ЭКРАН РЕЗУЛЬТАТОВ С СПОЙЛЕРАМИ"""
    ...
    # Проверяем формат профиля (новый с архетипом или старый)
    # Для CARD_DATA проверяем наличие поля 'archetype'
    is_new_format = 'archetype' in profile_card
    
    # ============================================
    # ✅ ОСНОВНОЙ ТЕКСТ СО СПОЙЛЕРАМИ
    # ============================================
    
    if is_new_format:
        # НОВЫЙ ФОРМАТ (с архетипом и раздельными полями)
        if not has_shared:
            # ЭКРАН 1: До шаринга
            profile_text = (
                f"<b>🎴 ТВОЙ ПРОФИЛЬ ОПРЕДЕЛЁН!</b>\n\n"
                f"<b>{profile_card.get('display_name', profile_card.get('title', 'Ваш профиль'))}</b>\n\n"
                f"<i>{profile_card.get('archetype', '')}</i>\n\n"
                f"<b>💬 КЛЮЧЕВАЯ ЦИТАТА:</b>\n"
                f"{profile_card.get('quote', '')}\n\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                f"<b>👇 НАЖМИ НА КАЖДЫЙ БЛОК, ЧТОБЫ РАСКРЫТЬ:</b>\n\n"
                f"<tg-spoiler><b>🔍 ЭТО ТЫ, ЕСЛИ...</b>\n"
                f"{profile_card.get('trigger', profile_card.get('pain', ''))[:120]}...</tg-spoiler>\n\n"
                
                f"<tg-spoiler><b>💔 СУТЬ ПРОБЛЕМЫ</b>\n"
                f"{profile_card.get('pain', '')[:120]}...</tg-spoiler>\n\n"
                
                f"<tg-spoiler><b>🛠 ИНСТРУМЕНТ «ПРЯМО СЕЙЧАС»</b>\n"
                f"{profile_card.get('immediate_tool', '')[:120]}...</tg-spoiler>\n\n"
                
                f"<tg-spoiler><b>🚀 ЧТО ДАЛЬШЕ?</b>\n"
                f"{profile_card.get('cta', '')[:120]}...</tg-spoiler>\n\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                f"💬 <i>Иногда самое большое, что мы можем сделать для близких — это дать зеркало...</i>\n"
                f"Поделись тестом и получи 🎁 <b>БЕСПЛАТНЫЙ ПОДАРОК</b>"
            )
        else:
        ...
        profile_text = (
            f"<b>🎴 ТВОЙ ПРОФИЛЬ ОПРЕДЕЛЁН!</b>\n\n"
            f"<b>{profile_card.get('title', 'Ваш профиль')}</b>\n\n"
            f"<tg-spoiler><b>🌍 ТВОЙ МИР</b>\n"
            f"{profile_card.get('world', '')[:120]}...</tg-spoiler>\n\n"
            
            f"<tg-spoiler><b>⚡️ ТВОЯ СУПЕРСИЛА</b>\n"
            f"{profile_card.get('superpower', '')[:120]}...</tg-spoiler>\n\n"
            
            f"<tg-spoiler><b>🚀 ТОЧКА РОСТА</b>\n"
            f"{profile_card.get('growth', '')[:120]}...</tg-spoiler>\n\n"
            
            f"<b>🎯 ЧТО ДЕЛАТЬ:</b>\n{profile_card.get('cta', '')}\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"💬 <i>Иногда самое большое, что мы можем сделать для близких — это дать зеркало...</i>\n"
            f"Поделись тестом и получи 🎁 <b>БЕСПЛАТНЫЙ ПОДАРОК</b>"
        )
    ...
    if not has_shared:
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("📄 Полный текст профиля", callback_data="full_profile_text")],
    ...
    try:
        ...
        await query.edit_message_text(
            "❌ Сообщение слишком длинное. Пожалуйста, используйте кнопку «📄 Полный текст профиля»",
            reply_markup=reply_markup
        )
...
# ============================================
# ✅ НОВАЯ ФУНКЦИЯ: ПОЛНЫЙ ТЕКСТ ПРОФИЛЯ
# ============================================

async def show_full_profile_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """✅ Показывает полный текст профиля без спойлеров"""
    query = update.callback_query
    await query.answer()
    ...
    if is_new_format:
        ...
        # Добавляем все блоки по порядку
        blocks = [
            ("🔍 ЭТО ТЫ, ЕСЛИ...", "trigger"),
            ("💔 СУТЬ ПРОБЛЕМЫ", "pain"), 
            ("🛠 ИНСТРУМЕНТ «ПРЯМО СЕЙЧАС»", "immediate_tool"),
            ("🚀 ЧТО ДАЛЬШЕ?", "cta")
        ]
        
        for title, key in blocks:
            if key in profile_card and profile_card[key]:
        ...
        # Добавляем CTA
        if 'cta' in profile_card and profile_card['cta']:
            full_text += f"<b>🎯 ЧТО ДЕЛАТЬ:</b>\n{profile_card['cta']}\n\n{'━'*30}\n\n"
    
    # Добавляем кнопку назад
    keyboard = [[InlineKeyboardButton("⬅️ Назад к результатам", callback_data="back_to_results")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    # Отправляем (Telegram ограничение 4096 символов)
...
# ============================================
# ✅ ОСТАЛЬНЫЕ ФУНКЦИИ ЭКРАНОВ (СОХРАНЕНЫ)
# ============================================
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            RESULTS: [
                ...
                # ✅ НОВЫЙ ОБРАБОТЧИК ДЛЯ ПОЛНОГО ТЕКСТА
                CallbackQueryHandler(show_full_profile_text, pattern="^full_profile_text$"),
    ...
    # Запуск бота
    logger.info("🚀 Bot started with NEW 5-screen navigation and Telegram spoilers!")
