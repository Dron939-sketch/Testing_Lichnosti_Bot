...
Версия: 11.0 - ПОЛНАЯ СИНХРОНИЗАЦИЯ КНОПОК
✅ ВСЕ CALLBACK_DATA УНИФИЦИРОВАНЫ
✅ КНОПКА "МОИ ОТРАЖЕНИЯ" = my_reflections
✅ КНОПКА "ИНТИМНЫЙ ПРОФИЛЬ" = my_sexual_profile
✅ КНОПКА "СОЗДАТЬ ССЫЛКУ" = create_invite
✅ КНОПКА "4F-КЛЮЧИ" = four_f_keys
✅ ИСПРАВЛЕН DEEP LINK
"""
...
def init_test_data(user_id: int):
    ...
    # Используем заведомо НЕРЕАЛЬНЫЕ ID для тестов
    import random
...
# ===== ЗАГРУЗКА ИНТИМНОГО ПРОФИЛЯ С КЭШЕМ =====
...
async def my_sexual_profile_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("💎 МОИ ОТРАЖЕНИЯ", callback_data="my_reflections")],  # ✅ УНИФИЦИРОВАНО!
...
async def my_reflections_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """💎 МОИ ОТРАЖЕНИЯ - УНИФИЦИРОВАННЫЙ ОБРАБОТЧИК!"""
    query = update.callback_query
    await query.answer()
...
# ============================================
# 🚀 ЗАПУСК БОТА - ИСПРАВЛЕННЫЙ CONVERSATION HANDLER
# ============================================

def main():
    """Запуск бота с ПОЛНОСТЬЮ УНИФИЦИРОВАННЫМИ callback_data"""
    print("\n" + "="*60)
    print("🔞 ИНТИМНЫЕ ПРОФИЛИ И 4F-КЛЮЧИ v11.0")
    print("="*60)
    print("✅ ПОЛНАЯ СИНХРОНИЗАЦИЯ КНОПОК:")
    print("   • my_sexual_profile - Интимный профиль")
    print("   • my_reflections - Мои отражения")
    print("   • create_invite - Создать ссылку")
    print("   • four_f_keys - 4F-ключи")
    print("✅ ИСПРАВЛЕН DEEP LINK")
    print("✅ ДОБАВЛЕНО КЭШИРОВАНИЕ")
    print("✅ ДОБАВЛЕНА ВАЛИДАЦИЯ CALLBACK_DATA")
    print("="*60)
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            FRIEND_MENU: [
                CallbackQueryHandler(standard_profile_callback, pattern='^standard_'),
                CallbackQueryHandler(intimate_profile_callback, pattern='^intimate_'),
                CallbackQueryHandler(four_f_menu_callback, pattern='^four_f_'),
    ...
    print("\n🚀 БОТ ЗАПУЩЕН!")
    print("✅ ВСЕ CALLBACK_DATA УНИФИЦИРОВАНЫ:")
    print("   • my_reflections - везде одинаково")
    print("   • my_sexual_profile - везде одинаково")
    print("   • create_invite - везде одинаково")
    print("   • four_f_ - везде одинаково")
    print("="*60)
