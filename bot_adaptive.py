...
# ============================================
# ✅ ИСПРАВЛЕНО: УРОВНИ ДИЛТСА (5 УРОВНЕЙ)
# ============================================
DILTS_LEVELS = {
    "ENVIRONMENT": {
        "name": "ОКРУЖЕНИЕ",
...
def determine_perception_type(scores):
    """
    Определяет тип восприятия по баллам ЭТАПА 1
    
    Возвращает один из 4 типов:
    - СОЦИАЛЬНО-АФФИЛИАТИВНЫЙ (SA): EXTERNAL + SYMBOLIC
    - ЭКЗИСТЕНЦИАЛЬНО-РЕФЛЕКСИВНЫЙ (IA): INTERNAL + SYMBOLIC
    - ИНСТРУМЕНТАЛЬНО-ДОСТИЖЕНЧЕСКИЙ (SP): EXTERNAL + MATERIAL
    - СТРУКТУРНО-АНАЛИТИЧЕСКИЙ (IP): INTERNAL + MATERIAL
    """
...
def calculate_thinking_level_optimized(level_scores):
    """
    ✅ ОПТИМИЗИРОВАННЫЙ АЛГОРИТМ: Определяет уровень мышления (1-9)
    
    Более точное распределение по 9 уровням:
    - Уровень 1: avg <= 1.3
    - Уровень 2: 1.3 < avg <= 1.8
    - Уровень 3: 1.8 < avg <= 2.5
    - Уровень 4: 2.5 < avg <= 3.2
    - Уровень 5: 3.2 < avg <= 4.0
    - Уровень 6: 4.0 < avg <= 4.5
    - Уровень 7: 4.5 < avg <= 4.8
    - Уровень 8: 4.8 < avg <= 5.2
    - Уровень 9: avg > 5.2
    """
    if not level_scores:
        return 1
...
def determine_dilts_level_smart(dilts_answers, thinking_level):
    """
    ✅ УМНЫЙ АЛГОРИТМ: Определяет проблемный уровень Дилтса с учётом уровня мышления
    
    Логика:
    1. Считаем самый частый ответ из ЭТАПА 4
    2. Корректируем по уровню мышления (низкие уровни не могут иметь высокие проблемы)
    
    Правила коррекции:
    - Уровень 1-2: максимум BEHAVIOR (не может быть VALUES/IDENTITY)
    - Уровень 3-5: максимум CAPABILITIES
    - Уровень 6-7: максимум VALUES
    - Уровень 8-9: может быть IDENTITY
    """
    if not dilts_answers:
        return "ENVIRONMENT"
    
    # Шаг 1: Определяем самый частый ответ
    counter = Counter(dilts_answers)
    most_common_dilts = counter.most_common(1)[0][0]
    
    # Шаг 2: Иерархия уровней Дилтса (от низкого к высокому)
    dilts_hierarchy = {
        "ENVIRONMENT": 1,
        "BEHAVIOR": 2,
        "CAPABILITIES": 3,
    ...
    # Шаг 3: Определяем максимально допустимый уровень Дилтса
    if thinking_level <= 2:
        max_allowed_dilts = "BEHAVIOR"
    elif thinking_level <= 5:
        max_allowed_dilts = "CAPABILITIES"
    elif thinking_level <= 7:
        max_allowed_dilts = "VALUES"
    else:
        max_allowed_dilts = "IDENTITY"
    
    # Шаг 4: Корректируем, если выбранный уровень слишком высокий
    if dilts_hierarchy.get(most_common_dilts, 1) > dilts_hierarchy.get(max_allowed_dilts, 1):
        logger.info(f"Dilts corrected: {most_common_dilts} → {max_allowed_dilts} (thinking_level={thinking_level})")
        return max_allowed_dilts
    
    return most_common_dilts
...
def get_dilts_code(dilts_level: str) -> str:
    """
    ✅ ИСПРАВЛЕНО: Возвращает код Дилтса (env/beh/cap/val/ide)
    """
    dilts_map = {
        "ENVIRONMENT": "env",
        "BEHAVIOR": "beh",
        "CAPABILITIES": "cap",
        "VALUES": "val",  # ✅ ИСПРАВЛЕНО
        "IDENTITY": "ide"  # ✅ ИСПРАВЛЕНО
    }
    return dilts_map.get(dilts_level, "env")

def calculate_profile_key(context_data: dict) -> str:
    """
    ✅ ГЛАВНАЯ ФУНКЦИЯ: Определяет ключ профиля в формате "SA_1_env"
    
    Формат: {TYPE_CODE}_{LEVEL}_{DILTS_CODE}
    
    Примеры:
    - SA_1_env (Социально-аффилиативный, уровень 1, проблема в окружении)
    - IP_5_cap (Структурно-аналитический, уровень 5, проблема в способностях)
    - SP_8_ide (Инструментально-достиженческий, уровень 8, проблема в идентичности)
    
    Args:
        context_data: словарь с данными пользователя
    
    Returns:
        строка вида "SA_1_env" (один из 36 профилей)
    """
    ...
    # 3. Получаем код Дилтса (env/beh/cap/val/ide)
    ...
    logger.info(f"✅ Profile calculated: {profile_key} (type={type_code}, level={level}, dilts={dilts_code})")
    return profile_key
...
async def finish_stage_2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """✅ ИСПРАВЛЕНО: Завершение ЭТАПА 2 с новым алгоритмом"""
    query = update.callback_query
    
    level_scores = context.user_data.get("stage2_level_scores", [])
    thinking_level = calculate_thinking_level_optimized(level_scores)  # ✅ НОВАЯ ФУНКЦИЯ
...
async def finish_stage_3(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ограничиваем диапазон 1-9
    final_level = max(1, min(9, final_level))
    
    context.user_data["final_level"] = final_level
    
    logger.info(f"User {update.effective_user.id}: Stage 3 complete, final_level={final_level} (stage2={stage2_level}, stage3_avg={stage3_avg:.2f})")
...
async def show_stage_4_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """✅ ИСПРАВЛЕНО: Детали ЭТАПА 4 (убрана МИССИЯ)"""
    query = update.callback_query
    await query.answer()
...
async def finish_stage_4(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """✅ ИСПРАВЛЕНО: Завершение ЭТАПА 4 с умным алгоритмом"""
    query = update.callback_query
    
    dilts_answers = context.user_data.get("stage4_dilts_answers", [])
    thinking_level = context.user_data.get("final_level", 1)
    
    # ✅ ИСПОЛЬЗУЕМ УМНЫЙ АЛГОРИТМ С КОРРЕКЦИЕЙ
    dilts_level = determine_dilts_level_smart(dilts_answers, thinking_level)
    context.user_data["dilts_level"] = dilts_level
    
    logger.info(f"User {update.effective_user.id}: Stage 4 complete, dilts={dilts_level}, thinking_level={thinking_level}")
...
def main():
    ...
    logger.info("✅ Бот запущен с новым алгоритмом определения 36 профилей!")
